 ...
 *
 * `KithRules.fetch()` was `Parse.Collection.fetch`, which is a bare
 * `new Parse.Query(model).find()` -- no limit, so the SDK's default of 100 rows,
 * which is more than the kith list has ever held. Kept as-is rather than paged,
 * because raising the limit would be a behaviour change hiding in a port.
 */
async function fetchKithRules(): Promise<KithRules> {
  const query = new Parse.Query(RULE_CLASS_NAMES.kith)
  return await query.find()
}

/**
 * The single shared engine -- `BNSCTDBS_ChangelingCostsFetcher.js`.
 *
 * The source memoised the *instance* and returned it before its fetch had
 * finished: `globalCosts` was assigned, then `initialize()` was called, so a
 * second caller arriving during the fetch took the `if (globalCosts)` branch and
 * got an engine whose kith rules were still empty -- which prices every affinity
 * Art at the non-affinity 6-per-level table instead of 4. A failed fetch left
 * that same empty instance memoised permanently.
 *
 * The promise is memoised here instead, so a second caller waits for the first
 * caller's fetch, and a rejection clears the memo so the next call retries. Both
 * differences are invisible to a caller that awaits before using the engine,
 * which is what `ChangelingBetaSlice.initialize_costs` does.
 */
let costsPromise: Promise<ChangelingBetaSliceCosts> | null = null

export function get_changeling_costs(): Promise<ChangelingBetaSliceCosts> {
  if (!costsPromise) {
    costsPromise = fetchKithRules()
      .then((rules) => new ChangelingBetaSliceCosts(rules))
      .catch((error: unknown) => {
        costsPromise = null
        throw error
      })
...
/**
 * Drop the memoised engine.
 *
 * For tests and for a sign-out that changes which server the next query talks
 * to. Nothing in the app calls it during normal use.
 */
export function reset_changeling_costs(): void {
  costsPromise = null
}
