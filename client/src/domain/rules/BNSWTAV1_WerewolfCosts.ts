...
export class WerewolfCosts {
  ...
  /**
   * Load the gift descriptions.
   *
   * Parse 1.5 also called `initialize` from the constructor, so this query ran
   * twice - once implicitly and once from `initialize_costs` - and both runs
   * added into the same collection, which deduplicated by id. A plain class has
   * no such hook, so it runs once. Same result, one fewer round trip.
   *
   * `each` rather than `find`: no limit, no ordering, every matching row.
   */
  async initialize(): Promise<void> {
    const loaded: GiftDescription[] = []
    const q = new Parse.Query(DescriptionObject).equalTo('category', 'wta_gifts')
    await q.each((d) => {
      loaded.push(d)
    })
    this.descriptions = loaded
  }
