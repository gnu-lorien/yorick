...
/**
 * Join Troupe -- `#character-pick-troupe-to-join`.
 *
 ...
 * (`mobileRouter.js:1913`). `TroupeList.vue` is that view; this page supplies
 * the two arguments.
 *
 ...
 * This one INVERTS the filter: only troupes the character is not already in
 * are offered.
 */
...
function hrefFor(troupeId: string) {
  return `#character/${cid.value}/troupe/${troupeId}/join`
}
...
<template>
  <JqmPage id="character-pick-troupe-to-join" title="Join Troupe" :ready="loaded">
    <div role="troupe-list">
      <TroupeList :troupes="troupes" :href-for="hrefFor" filter-id="character-pick-troupe-to-join-filter" />
    </div>
  </JqmPage>
</template>
