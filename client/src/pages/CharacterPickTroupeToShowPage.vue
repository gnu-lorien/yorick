...
/**
 * Show Troupe -- `#character-pick-troupe-to-show`.
 *
 ...
 * (`mobileRouter.js:1873`). `TroupeList.vue` is that view; this page supplies
 * the two arguments.
 *
 ...
 * A character in no troupe sees an empty list, which is correct and is what
 * the source rendered.
 */
...
function hrefFor(troupeId: string) {
  return `#character/${cid.value}/troupe/${troupeId}/show`
}
...
<template>
  <JqmPage id="character-pick-troupe-to-show" title="Show Troupe" :ready="loaded">
    <div role="troupe-list">
      <TroupeList :troupes="troupes" :href-for="hrefFor" filter-id="character-pick-troupe-to-show-filter" />
    </div>
  </JqmPage>
</template>
