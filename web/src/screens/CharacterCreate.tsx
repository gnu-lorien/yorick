...
export function CharacterCreate({ route }: ScreenProps) {
  ...
  // Back to where the player was before they went off to pick. `consume: false`
  // because `backToTop` lives on the router's memoised view for the life of the
  // page, so the legacy restores the last recorded position on any return to
  // the wizard. See shell/scrollMemory.ts.
  useScrollMemory({ key: 'character-create', ready, consume: false });
