...
describe('A Vampire\'s costs', () => {
  ...
  it('has base Ventrue in clan disciplines', async () => {
    ...
    const icds = await vampire.value.get_in_clan_disciplines()
  ...
  it('no longer has extra in clans after removal', async () => {
    ...
    expect(icds).to.include('MadeUpForTesting')
    const extra_discipline = vampire.value.get_trait_by_name('extra_in_clan_disciplines', 'MadeUpForTesting')
    await vampire.value.remove_trait(extra_discipline)
